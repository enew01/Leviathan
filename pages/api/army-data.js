...
export default function handler(req, res) {
    ...
    if (method !== "GET" || !query.name) {
        res.status(422).json({ status: "failed", message: "Please use correct method and query param." });
        return;
    }
    const armyCardDir = path.join(process.cwd(), `public/cards/${query.name}`);
    if (!fs.existsSync(armyCardDir)) {
        res.status(433).json({ status: "failed", message: "please indicate correct directory" });
    }
